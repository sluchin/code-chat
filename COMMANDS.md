...
> **実装状況**: `-p/--provider` は `gemini` のみ対応です。それ以外の値は未実装のため、引数のエラーになります。
...
| `-p, --provider` | 文字列 | `gemini` | LLM プロバイダ。現在は `gemini` のみ (他の値は未実装のため、引数のエラーになる) |
...
| `code-chat cache update` | `[PATH]` `--ttl <SECONDS>` | 同じパスの既存キャッシュを削除し、最新の内容で作り直す (内容は更新できないため再作成。既存の TTL は引き継がず、`--ttl` で指定した値 (デフォルト: 3600秒) になる)。既存がなければ新規作成 |

...
from code_chat.logger import setup_logging
...
def test_readline_import_primary_success():
    ...
    with patch.dict("sys.modules", {"readline": MagicMock()}):
        import code_chat.chat  # 対象のモジュール名に調整してください

        importlib.reload(code_chat.chat)
        assert code_chat.chat.readline is not None
...
def test_readline_import_fallback_pyreadline3_success():
    ...
    with patch("builtins.__import__", side_effect=mock_import):
        with patch.dict("sys.modules", {"pyreadline3": mock_pyreadline3}):
            import code_chat.chat

            importlib.reload(code_chat.chat)
            assert code_chat.chat.readline is mock_pyreadline3
...
def test_readline_import_both_failed():
    ...
    with patch("builtins.__import__", side_effect=mock_import):
        ...
        importlib.reload(code_chat.chat)
        assert code_chat.chat.readline is None
...
def test_setup_readline_history_os_error():
    ...
    with patch("code_chat.chat.readline", mock_readline), patch(
        "code_chat.chat.HISTORY_FILE", mock_history_file
    ):

        from code_chat.chat import setup_readline_history
...
def test_save_readline_history_os_error():
    ...
    with patch("code_chat.chat.readline", mock_readline), patch(
        ...
        from code_chat.chat import save_readline_history
...
def test_handle_commit_msg_generation_suppresses_traceback(caplog):
    ...
    # git diff のダミー値を返す設定
    with patch("code_chat.chat.get_git_diff", return_value="diff --git a/b"):
        # Gemini API の呼び出しで Exception を発生させる
        with patch(
            "code_chat.chat.send_message_stream_with_retry",
...
def test_main_generate_commit_msg_failure_exits_with_code_1():
    ...
    with patch.object(sys, "argv", test_args):
        with patch("code_chat.chat.parse_args") as mock_parse_args:
            ...
            with patch(
                "code_chat.chat.handle_commit_msg_generation",
...
def test_main_unexpected_exception_exc_info_logging(caplog):
    ...
    with patch.object(sys, "argv", test_args):
        with patch("code_chat.chat.parse_args", side_effect=Exception("Fatal Boom")):
            with pytest.raises(SystemExit) as exc_info:
                main()
...
def test_main_keyboard_interrupt_exit():
    ...
    with patch("code_chat.chat.parse_args") as mock_args, patch(
        "code_chat.chat.get_gemini_client"
    ), patch(
        "code_chat.chat.run_interactive_loop", side_effect=KeyboardInterrupt
    ):
        ...
        from code_chat.chat import main
...
def test_main_eof_error_exit():
    ...
    with patch("code_chat.chat.parse_args") as mock_args, patch(
        "code_chat.chat.get_gemini_client"
    ), patch("code_chat.chat.run_interactive_loop", side_effect=EOFError):
        mock_args.return_value = MagicMock(
            list_models=False,
            generate_commit_msg=False,
            debug=False,

...
3. [code-chat のリトライ動作とエラー出力](#3-code-chat-のリトライ動作)
...
- **code-chat の動作**: 自動でリトライします (初回を含めて最大 3 回試行し、待ち時間は 1 秒から 2 倍ずつ)。詳しくは [3 章](#3-code-chat-のリトライ動作)。
...
### 3.1 通常の対話・ワンショット・コミットメッセージ生成・コードレビュー

`send_message_with_retry` / `send_message_stream_with_retry` が、リトライを担当します。`--review` も、同じリトライを使います。

| 項目 | 内容 |
| --- | --- |
| リトライの対象 | HTTP 503 / 429、またはメッセージに `503` / `UNAVAILABLE` / `429` / `RESOURCE_EXHAUSTED` を含むエラー |
| リトライしない | 上記以外 (400、403、404 など)、および **1 日あたりの上限** (メッセージに `PerDay` を含む 429) |
| 回数 | 初回を含めて最大 3 回試行する (リトライは最大 2 回) |
| 待ち時間 | 1 秒から始めて、2 倍ずつ増やす (1 回目のリトライまで 1 秒、2 回目まで 2 秒) |
| 待ち時間の優先 | エラーに `retryDelay` (例: `retryDelay: '30s'`) が含まれる場合は、その秒数 + 1 秒 |
| リトライ中の表示 | `Gemini API で一時的なエラーが発生しました (1/3): [HTTP 429 RESOURCE_EXHAUSTED] 1 分あたりの ... 2.0秒後に再試行します...` |
| ストリーミング | 応答の出力が始まったあとにエラーになった場合は、出力の重複を避けるため、**リトライしない** |
| 最終的に失敗 | 概要とヒントを 1 回だけ出力して、終了コード 1 で終了する ([3.4](#34-エラーの表示形式)) |

### 3.2 MCP 連携 (`--mcp`)

`QueryHandler` が、429 (レート制限) のときだけ、最大 5 回まで自動で試行します (待ち時間は、指数的に増やし、5 秒から 60 秒まで)。

- **1 日あたりの上限**は、待っても回復しないため、リトライしません。
- **リトライの待機に入る前に**、原因と回数を警告として出力します (例: `Gemini API のレート制限のため, 5 秒後に再試行します (1/5): [HTTP 429 ...]`)。
- 429 以外のエラーは、リトライしません。
- 最終的に失敗した場合は、概要とヒントを出力します。対話モードでは、そのまま次の入力に進みます。
...
Gemini API 以外の例外 (ファイル操作、内部のバグなど) は、`--trace` なしでも、常にトレースバックを出力します。
...
| `-c/--cache は --mcp と併用できません` など | 引数の組み合わせが不正 (終了コード 2)。`-c` は、`--mcp` / `-w` / `--oauth` と併用できない |
